...
#define PORTNUM2 5678UL
#define PORTNUM 1234UL
 ...
 */
static command_error_t gpio_read_handler(const uint8_t * buffer, size_t len, char* msg)
{
	int pin_num;
	char gpio_port;
 ...
 * size_t len - size of buffer
 */
static command_error_t gpio_write_handler(const uint8_t * buffer, size_t len){
	char gpio_port;
	int pin_state;
	int pin_num;
		...
		else if (retval)
		{
			...
			if (FD_ISSET(socket_fd, &rfds)){
				...
				if (received > 0){
					...
					if ( (r = gpio_read_handler(buffer, received, gpio_status_feedback)) != COMMAND_OK){
			...
			if (FD_ISSET(socket2_fd, &rfds)){
				...
				if (received2 > 0){
					...
					if ( (r = gpio_write_handler(buffer2, received2)) != COMMAND_OK){
